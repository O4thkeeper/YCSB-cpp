...
const std::string PROP_DISABLE_WAL = "rocksdb.disable_wal";
...
const std::string PROP_ENABLE_BLOB_GARBAGE_COLLECTION =
    "rocksdb.enable_blob_garbage_collection";
const std::string PROP_ENABLE_BLOB_GARBAGE_COLLECTION_DEFAULT = "false";

const std::string PROP_BLOB_GARBAGE_COLLECTION_AGE_CUTOFF =
    "rocksdb.blob_garbage_collection_age_cutoff";
const std::string PROP_BLOB_GARBAGE_COLLECTION_AGE_CUTOFF_DEFAULT = "0.25";

const std::string PROP_BLOB_GARBAGE_COLLECTION_FORCE_THRESHOLD =
    "rocksdb.blob_garbage_collection_force_threshold";
const std::string PROP_BLOB_GARBAGE_COLLECTION_FORCE_THRESHOLD_DEFAULT = "1.0";

const std::string PROP_BLOB_COMPACTION_READAHEAD_SIZE =
    "rocksdb.blob_compaction_readahead_size";
const std::string PROP_BLOB_COMPACTION_READAHEAD_SIZE_DEFAULT = "0";

const std::string PROP_BLOB_FILE_SIZE = "rocksdb.blob_file_size";
const std::string PROP_BLOB_FILE_SIZE_DEFAULT = "134217728";
...
void RocksdbDB::GetOptions(
  ...
  if (options_file != "") {
    ...
    if (props.GetProperty(PROP_ENABLE_BLOB_FILES,
                          PROP_ENABLE_BLOB_FILES_DEFAULT) == "true") {
      opt->enable_blob_files = true;
    }
    val = std::stoi(
        props.GetProperty(PROP_MIN_BLOB_SIZE, PROP_MIN_BLOB_SIZE_DEFAULT));
    opt->min_blob_size = val;
    if (props.GetProperty(PROP_ENABLE_BLOB_GARBAGE_COLLECTION,
                          PROP_ENABLE_BLOB_GARBAGE_COLLECTION_DEFAULT) ==
        "true") {
      opt->enable_blob_garbage_collection = true;
    }
    double d_val = std::stod(
        props.GetProperty(PROP_BLOB_GARBAGE_COLLECTION_AGE_CUTOFF,
                          PROP_BLOB_GARBAGE_COLLECTION_AGE_CUTOFF_DEFAULT));
    opt->blob_garbage_collection_age_cutoff = d_val;
    d_val = std::stod(props.GetProperty(
        PROP_BLOB_GARBAGE_COLLECTION_FORCE_THRESHOLD,
        PROP_BLOB_GARBAGE_COLLECTION_FORCE_THRESHOLD_DEFAULT));
    opt->blob_garbage_collection_force_threshold = d_val;
    val = std::stoi(
        props.GetProperty(PROP_BLOB_COMPACTION_READAHEAD_SIZE,
                          PROP_BLOB_COMPACTION_READAHEAD_SIZE_DEFAULT));
    if (val != 0) {
      opt->blob_compaction_readahead_size = val;
    }
    val = std::stoi(
        props.GetProperty(PROP_BLOB_FILE_SIZE, PROP_BLOB_FILE_SIZE_DEFAULT));
    if (val != 0) {
